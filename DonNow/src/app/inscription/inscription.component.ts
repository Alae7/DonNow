  ...
  onSubmit() {
    if (this.formData.terms) {
      if (!this.selectedFile) {
        this.showPopup("Veuillez choisir une image.", false);
        return;
      }
      ...
      this.http.post(url, formData).subscribe({
        ...
        error: (error) => {
          ...
          if (error.error) {
            this.showPopup('Erreur : ' + error.error, false);
          } else {
            this.showPopup('Une erreur est survenue. Veuillez réessayer.', false);
          }
        }
      });
    } else {
      this.showPopup('Vous devez accepter les conditions d’utilisation.', false);
    }
  }
