...
export class DonateComponent implements OnInit {
  ...
  copyLink() {
    ...
    navigator.clipboard.writeText(link).then(() => {
      // Instead of alert, call showPopup to display a modal
      this.showPopup("Link copied to clipboard!", false); // Passing false for isError (indicating success)
    });
  }
...
Sub :any;
  ...
  ngOnInit(): void {
    ...
    this.route.url
      ...
      .subscribe((urlPath) => {
        if (urlPath.includes('/unsuccessful')) {
          this.showPopup('Donation unsuccessful. Please try again.', true);
        } else if (urlPath.includes('/successful')) {
